    ...
    def _load_model(self, model_id):  # noqa: C901
        ...
        if model_config["type"] == "yolov5":
            ...
            try:
                ...
                return
        elif model_config["type"] == "comic_text_detector":
            from .comic_text_detector import ComicTextDetector

            try:
                model_config["model"] = ComicTextDetector(
                    model_config, on_message=self.new_model_status.emit
                )
                self.auto_segmentation_model_unselected.emit()
                logger.info(
            ...
            try:
                ...
                return
        elif model_config["type"] == "koharu_rfdetr_seg":
            from .koharu_rfdetr_seg import KoharuRFDETRSeg

            try:
                model_config["model"] = KoharuRFDETRSeg(
                    model_config, on_message=self.new_model_status.emit
                )
                self.auto_segmentation_model_unselected.emit()
                logger.info(
